...
procedure Check_Google_Drive_Compatibility is
   ...
   Fixture    : constant String :=
     Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_FIXTURE", "./tests/bin/backup_http_remote_live_tests");
   Remote     : constant String := Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_REMOTE");
   Strict     : constant Boolean :=
     Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_STRICT") = "1"
     or else Backup_Tool_Support.Env ("CI") = "true";
...
begin
   ...
   declare
      ...
      Text : constant String :=
        ...
        (if Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_API_BASE") /= "" then "google_drive_api_base="
           & Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_API_BASE")
           & ASCII.LF else "")
           &
        (if Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_UPLOAD_BASE") /= "" then "google_drive_upload_base="
           & Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_UPLOAD_BASE")
           & ASCII.LF else "")
           &
        (if Token_File /= "" then "google_drive_access_token_file=" & Token_File & ASCII.LF else "") &
        (if Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_REFRESH_TOKEN") /= ""
         then "google_drive_refresh_token="
            & Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_REFRESH_TOKEN")
            & ASCII.LF
         else "") &
        (if Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_CLIENT_ID") /= "" then "google_drive_client_id="
           & Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_CLIENT_ID")
           & ASCII.LF else "")
           &
        (if Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_CLIENT_SECRET") /= ""
         then "google_drive_client_secret="
            & Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_CLIENT_SECRET")
            & ASCII.LF
         else "") &
        (if Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_DRIVE_ID") /= "" then "google_drive_drive_id="
           & Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_DRIVE_ID")
           & ASCII.LF
   ...
   begin
      if Token_File = "" and then Backup_Tool_Support.Env ("BACKUP_GOOGLE_DRIVE_COMPAT_REFRESH_TOKEN") = "" then
         Backup_Tool_Support.Fail ("Google Drive compatibility credentials are required: set BACKUP_GOOGLE_DRIVE_CO"
                                   & "MPAT_TOKEN_FILE or BACKUP_GOOGLE_DRIVE_COMPAT_REFRESH_TOKEN");
      end if;
