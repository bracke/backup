...
procedure Check_Proton_Drive_Compatibility is
   ...
   Fixture    : constant String :=
     Backup_Tool_Support.Env ("BACKUP_PROTON_DRIVE_COMPAT_FIXTURE", "./tests/bin/backup_http_remote_live_tests");
   Remote     : constant String := Backup_Tool_Support.Env ("BACKUP_PROTON_DRIVE_COMPAT_REMOTE");
   Strict     : constant Boolean :=
     Backup_Tool_Support.Env ("BACKUP_PROTON_DRIVE_COMPAT_STRICT") = "1"
...
begin
   ...
   declare
      ...
      Text : constant String :=
        ...
        (if Backup_Tool_Support.Env ("BACKUP_PROTON_DRIVE_COMPAT_API_BASE") /= "" then "proton_drive_api_base="
           & Backup_Tool_Support.Env ("BACKUP_PROTON_DRIVE_COMPAT_API_BASE")
           & ASCII.LF else "")
           &
        (if Session_File /= "" then "proton_drive_session_file=" & Session_File & ASCII.LF else "") &
        (if Backup_Tool_Support.Env ("BACKUP_PROTON_DRIVE_COMPAT_USER_ADDRESS") /= "" then "proton_drive_user_address="
           & Backup_Tool_Support.Env ("BACKUP_PROTON_DRIVE_COMPAT_USER_ADDRESS")
           & ASCII.LF else "")
           &
        (if Backup_Tool_Support.Env ("BACKUP_PROTON_DRIVE_COMPAT_OPERATION_BASE_URL") /= ""
         then "proton_drive_operation_base_url="
            & Backup_Tool_Support.Env ("BACKUP_PROTON_DRIVE_COMPAT_OPERATION_BASE_URL")
            & ASCII.LF
         else "") &
   ...
   begin
      if Session_File = "" then
         Backup_Tool_Support.Fail ("Proton Drive compatibility credentials are required: set BACKUP_PROTON_DRIVE_CO"
                                   & "MPAT_SESSION_FILE");
      end if;
